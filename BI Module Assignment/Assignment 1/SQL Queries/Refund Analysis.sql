SELECT * FROM RefundFact rf;
SELECT * FROM TicketDim td ;
SELECT * FROM RefundDim rd ;
SELECT * FROM EventDim ed ;
SELECT * FROM EventFact ef ;
SELECT * FROM MerchandiseDim md ;


SELECT
	CONCAT(td.TicketEvent, td.TicketType), 
FROM
	EventFact ef
LEFT JOIN TicketDim td ON
	ef.TicketID = td.TicketID
LEFT JOIN RefundFact rf ON
	td.TicketID = rf.TicketID
JOIN MerchandiseDim mdef ON
	ef.MerchandiseID = mdef.MerchandiseID
JOIN MerchandiseDim mdrf ON
	rf.MerchandiseID = mdrf.MerchandiseID; 


SELECT 
	  CONCAT(UPPER(ed.EventName), '-', ed.EventYear) AS [Event Name]
	, CONCAT(td.TicketEvent, ' ', td.TicketType) AS TicketName
	, SUM(ef.TicketsSold) AS TotalTicketsSold
	, COALESCE(SUM(rf.TicketsRefunded)
	, 0) AS TotalTicketsRefunded
	, COALESCE(SUM(rf.TicketsRefundedPND)
	, 0) AS TotalRefundedPND
	, CASE WHEN SUM(ef.TicketsSold) = 0 THEN 0 ELSE (COALESCE(SUM(rf.TicketsRefunded), 0) * 1.0 / SUM(ef.TicketsSold)) * 100 END AS RateOfRefundPercentagePerTicket
	, CASE WHEN SUM(ef.TicketsSoldPND) = 0 THEN 0 ELSE  (COALESCE(SUM(rf.TicketsRefundedPND), 0) * 1.0 / SUM(ef.TicketsSoldPND)) * 100 END AS RateOfRefundPercentagePerTicketPND
FROM EventFact ef 
LEFT JOIN TicketDim td ON ef.TicketID = td.TicketID
LEFT JOIN RefundFact rf ON ef.TicketID = rf.TicketID
LEFT JOIN EventDim ed ON ef.EventID = ed.EventID
GROUP BY
	ed.EventName, ed.EventYear, td.TicketEvent, td.TicketType
ORDER BY [Event Name], TicketName;
...
SELECT 
    CONCAT(UPPER(ed.EventName), '-', ed.EventYear) AS [Event Name],
    MerchandiseType,
    SUM(ef.MerchandiseSold) AS TotalMerchandiseSold,
    COALESCE(SUM(rf.MerchandiseRefunded), 0) AS TotalMerchandiseRefunded,
    COALESCE(SUM(rf.MerchandiseRefundedPND), 0) AS TotalRefundedPND,
    CASE 
        WHEN SUM(ef.MerchandiseSold) = 0 THEN 0 
        ELSE (COALESCE(SUM(rf.MerchandiseRefunded), 0) * 1.0 / SUM(ef.MerchandiseSold)) * 100 
    END AS RateOfRefundPercentagePerMerchandise,
    CASE 
        WHEN SUM(ef.MerchandiseSoldPND) = 0 THEN 0 
        ELSE (COALESCE(SUM(rf.MerchandiseRefundedPND), 0) * 1.0 / SUM(ef.MerchandiseSoldPND)) * 100 
    END AS RateOfRefundPercentagePerMerchandisePND
FROM 
    EventFact ef 
LEFT JOIN 
    MerchandiseDim md ON ef.MerchandiseID = md.MerchandiseID
LEFT JOIN 
    RefundFact rf ON ef.TicketID = rf.TicketID
LEFT JOIN 
    EventDim ed ON ef.EventID = ed.EventID
GROUP BY 
    ed.EventName, ed.EventYear, md.MerchandiseType
ORDER BY 
    [Event Name], MerchandiseType;
